<?php
defined('BASEPATH') or exit('No direct script access allowed');

class Leave_types extends AdminController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('hr_module/Leave_model');
    }

    public function index()
    {
        if (staff_cant('view', 'hr_leave')) {
            access_denied('hr_leave');
        }
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data(module_views_path('hr_module', 'leave_types/table'));
        }
        $data['title'] = _l('hr_leave_types_list');
        $this->load->view('hr_module/leave_types/index', $data);
    }

    public function add()
    {
        if (staff_cant('create', 'hr_leave')) {
            access_denied('hr_leave');
        }
        if ($this->input->post()) {
            $id = $this->Leave_model->add_type($this->_post_data());
            if ($id) {
                set_alert('success', _l('hr_leave_type_added'));
                redirect(admin_url('hr_module/leave_types'));
            }
            set_alert('danger', _l('hr_error_save_failed'));
            redirect(admin_url('hr_module/leave_types/add'));
        }

        $data['title']      = _l('hr_leave_type_add');
        $data['leave_type'] = null;
        $this->load->view('hr_module/leave_types/form', $data);
    }

    public function edit($id)
    {
        if (staff_cant('edit', 'hr_leave')) {
            access_denied('hr_leave');
        }
        $leave_type = $this->Leave_model->get_type($id);
        if (!$leave_type) show_404();

        if ($this->input->post()) {
            $this->Leave_model->update_type($this->_post_data(), $id);
            set_alert('success', _l('hr_leave_type_updated'));
            redirect(admin_url('hr_module/leave_types'));
        }

        $data['title']      = _l('hr_leave_type_edit');
        $data['leave_type'] = $leave_type;
        $this->load->view('hr_module/leave_types/form', $data);
    }

    public function delete($id)
    {
        if (staff_cant('delete', 'hr_leave')) {
            access_denied('hr_leave');
        }
        $result = $this->Leave_model->delete_type($id);
        if ($result['success']) {
            set_alert('success', _l('hr_leave_type_deleted'));
        } else {
            set_alert('danger', $result['message']);
        }
        redirect(admin_url('hr_module/leave_types'));
    }

    private function _post_data()
    {
        return [
            'name'                   => $this->input->post('name', true),
            'days_per_year'          => (int) $this->input->post('days_per_year'),
            'hours_per_day'          => (float) $this->input->post('hours_per_day') ?: 8.0,
            'carry_forward'          => $this->input->post('carry_forward') ? 1 : 0,
            'max_carry_forward_days' => (int) $this->input->post('max_carry_forward_days'),
            'requires_attachment'    => $this->input->post('requires_attachment') ? 1 : 0,
            'allow_half_day'         => $this->input->post('allow_half_day') ? 1 : 0,
            'is_date_range'          => $this->input->post('is_date_range') ? 1 : 0,
            'gender'                 => $this->input->post('gender') ?: null,
            'description'            => $this->input->post('description', true),
            'status'                 => $this->input->post('status') ? 1 : 0,
        ];
    }
}
